...
} from '../services/pokemonServices/pokemonSlice';
...
const PokeBase = () => {
  ...
  const [detailsId, setDetailsId] = useState(
    Math.ceil(Math.random() * 19).toString()
  );
  ...
  const handleNext = () => {
    setIsLoading(true);
    setOffset(
      (prev) => offset + 20 <= responseInfo.currentData?.count && prev + 20
    );
    window.scrollTo({ top: 0, left: 0, behavior: 'smooth' });
  };
  ...
  if (responseInfo.isError)
    return (
      <h2 className='error-heading'>
        An error occurred, {responseInfo.error.error}
      </h2>
    );
